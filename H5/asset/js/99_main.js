...
var car2 = {
	...
	plugin: function() {
		...
		// 音符飘逸
		$('#coffee_flow').coffee({
			steams: ["<img src='asset/images/audio_widget_01@2x.png' />", "<img src='asset/images/audio_widget_01@2x.png' />"],
			steamHeight: 100,
			steamWidth: 44
		});
	...
	init: function() {
		...
		$(window).on('load', function() {
			...
			//显示隐藏开始
			$(".ico").click(function() {
				var imgSrc = $(this).attr("dimg"); //获取大图src
				$(".floatDiv img").attr("src", imgSrc);
				$(".floatDiv").show()
			});
			$(".floatDiv .close").click(function() {
				$(this).parent().find("img").attr("src", "");
				$(this).parent().addClass("closeTop");
				$(this).parent().hide();
			});
			//鹅菜单显隐
			$(".e img").click(function() {
				var status = $(this).parent().find("ul").css("height");
				if (status == "336px") {
					$(this).parent().find("ul").animate({
						"height": 0
					}, 500);
					$(this).parent().find("ul").removeClass("flex");
				} else {
					$(this).parent().find("ul").animate({
						"height": 336
					}, 500);
			...
			//关闭层二及遮罩层
			$(".floatDiv2 .close").click(function() {
				$(this).parent().css({
					"zIndex": -10,
					"opacity": 0
				});
			...
			//计算弹出层高度;
			var floatDiv2H = $(".floatDiv2").height();
//			alert (floatDiv2H)870
			var touchSlide = $(".floatDiv2 .touchSlide").height();
//			alert (touchSlide)316
			var h2H = $(".content-wrap h2").height();
//			alert (h2H)100
			var contentWrapH = floatDiv2H - touchSlide - 40;
			var contentH = floatDiv2H - touchSlide - h2H - 40;

			$(".floatDiv2 .content-wrap").css("height", contentWrapH);
			$(".floatDiv2 .content").css("height", contentH);
			$(".slideBox").css("height", floatDiv2H);
